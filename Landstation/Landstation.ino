...
static const float WAIT_FOR_CYCLE_END = 5;
...
void loop()
{
  ...
  bool recived = buoy.receive_data(receivedData);
  if (recived)
  {
    if (receivedData.buoy_number > 0 && recived)
    {
    ...
    if (WAIT_FOR_CYCLE_END < millis() - fakeWiFiTimer || 1500000 < millis() - maximumWiFiTimer)
    {

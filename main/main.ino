...
void loop() {
  ...
  if (receivedData.buoy_number > 0 && !receivedData.alarm) {
    int idAlreadySeen = 0;
    // Check here if you have seen the buoy before
    for (int i = 0; i < BUOY_AMOUNT; i++) {
      if (idCheck[i] == receivedData.buoy_number) {
        idAlreadySeen = 1;
      }      
    }
    ...
    if (idAlreadySeen == 0) {
      ...
      double lat_diff_meters = (receivedData.gps_latitude - location[receivedData.buoy_number - 1][0]) * METERS_PER_DEGREE_LAT;

      // Longitude degree per meter changes from how far up you are, use original location to get a guesstimate
      double lon_diff_meters = (receivedData.gps_longitude - location[receivedData.buoy_number - 1][1]) * metersPerDegreeLon(location[receivedData.buoy_number - 1][1]);
  ...
  if (receivedData.buoy_number > 0 && receivedData.alarm){
    Serial.print("Alarm! Buoy ");
    Serial.print(receivedData.buoy_number);
    Serial.println(" has been sent adrift!");
    fakeWiFiTimer = millis();
    receivedData.buoy_number = 0;
    Serial.println("The Buoy's lat:");
    Serial.println(receivedData.gps_latitude);
    Serial.println("The Buoy's long:");
    Serial.println(receivedData.gps_longitude);
  }
